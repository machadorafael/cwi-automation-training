package br.com.sicredi.automation_web.pages;
...
import br.com.sicredi.automation_web.util.EvidenceReport;
	...
	protected boolean assertImageExists(String imageFile, float similarity0to100) {
		try {
			FileUtils.copyFile(new File(ImagePath.find(imageFile).getPath()),
					new File("target/relatorios/img/" + imageFile));
			extentTest.info("Imagem esperada: " + imageFile,
					MediaEntityBuilder.createScreenCaptureFromPath("../relatorios/img/" + imageFile).build());
		} catch (IOException e) {
